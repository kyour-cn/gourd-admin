...
const apiObj = properties.$API.system.system.role.list;
...
const tableDel = async (row) => {
	const res = await properties.$API.system.system.role.delete.post({
		ids: row.id
	});
	if (res.code === 0) {
		table.value.upData();
...
const batchDel = async () => {
	...
	const res = await properties.$API.system.system.role.del.post({ids});
	if (res.code === 0) {
